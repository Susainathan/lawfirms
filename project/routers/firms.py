...
@router.get("/law", status_code=status.HTTP_200_OK)
...
@router.patch("/law/{id}")
def update_keyfirm(id: int, updated_firm: schemas.UpdateHierarchy, db: Session = Depends(get_db)):
    firm_query = db.query(models.Law_Firm).filter(models.Law_Firm.id == id)
    post = firm_query.first()
    if post == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"firm with id: {id} does not exist")
...
@router.delete("/law/{id}", status_code=status.HTTP_202_ACCEPTED)
